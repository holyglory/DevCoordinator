...
The selected design source was the user-approved **Incident Workspace** concept.
The matching implementation capture is generated by
`MainBoardVerticalLayoutTests.testTypedStaleRetirementFailureRendersActionScopedRecoveryWorkspace`
at `.build/qa/retirement-center/ops-console-retirement-stale-1592x842.png`.
The combined top-reference/bottom-implementation review is written to
`.build/qa/retirement-center/design-comparison-final.png`; working comparison
images remain uncommitted by policy.

- Viewport: both comparison halves normalized to 1592 × 842 pixels. The native
  test renderer emits one raster pixel per requested layout point.
- State: a confirmed standalone-resource retirement is rejected before mutation
  with `lifecycle_plan_stale`; the confirmation sheet is closed, Activity is
  selected, the failed operation is selected, and **Refresh & re-plan** is
  available for that exact action.
- Full-shell review: fixed toolbar, concise incident header, operation list,
  readable decision guidance, technical evidence, inspector, and status footer
  remain separated and uncropped. The center owns one vertical scroller.
- Focused review: 524 × 760 and 784 × 900 center-pane captures preserve the
  recovery action, selected incident, next steps, selectable technical evidence,
  Copy action, and footer without overlap.
- P0: none. P1: none. P2: none. P3: the deterministic refreshed inventory
  intentionally retains only the exact unassigned host resource and omits an
  authoritative repository hierarchy, so the shell shows the honest hierarchy
  warning and no inspector selection; this does not affect the incident workflow
  under review.

Iteration history: the first implementation capture used a generic restart
failure and exposed technical evidence through a SwiftUI text surface. The final
iteration uses native read-only selectable text, a native accessible Copy button,
one store-owned Activity selection, action-scoped recovery, and the exact typed
stale-retirement failure. Its test then exercises **Refresh & re-plan** through a
second authoritative refresh and proves that a replacement plan uses the new
