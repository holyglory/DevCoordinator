...
| `POST /api/lifecycle/restore` | Owner-only `{ target_kind, target_id, reason? }`; exact row must be archived with `restorable:true`. Sends the Coordinator's exact three-field restore contract; the owner remains attributable in the normalized reason and authenticated Console audit context. Success means the fence was cleared, never that the resource started. |
