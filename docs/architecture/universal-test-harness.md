...
## S1 simplification boundary

S1 removes completed pre-availability cutover paths, duplicate legacy
authority/import paths, and compatibility-only state machinery. Testd and its
isolated Test Store remain the sole semantic authority for plans, runs,
attempts, deadlines, ordered results, terminal conclusions, and leases; the
privileged boundary reports exact host facts and performs only idempotent
prepare, start, observe, stop, and collect actions.

This simplification does not remove or rename the stable `enqueue`, reviewed
`submit`, `follow`, `queue-status`, `failures`, `cases`, `artifact`,
`artifact-export`, `cancel`, or `retry` actions. It also does not weaken
same-schema active-attempt recovery, bounded diagnostic continuations,
verified artifact export, exact identity and generation fencing, path
containment, non-root execution, cgroup isolation, TTL cleanup, idempotency,
or separate secret transport. Incompatible Test Store history remains
disposable and activates fresh through the documented cutover.
