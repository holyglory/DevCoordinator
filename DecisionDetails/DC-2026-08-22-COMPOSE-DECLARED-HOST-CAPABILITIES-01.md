...
either Compose host-mount representation or `cap_add` created repeated
operational friction without separating mutually distrusting users or
repositories. The user explicitly removed those three approval requirements on
2026-08-22.

## Decision

`host_bind_mount` (the service-level bind-mount classification),
`volume_driver_bind` (the equivalent local volume-driver bind classification),
and `added_capabilities` remain truthful effective-model risk classifications
and remain part of immutable model evidence, but they are not approval-required
risks. A model containing any combination can be sealed, persisted, reconciled
and started without an approval row, approval flag, wrapper call or additional
prompt.
...
It also does not change the approval boundary for non-loopback/public
publication, devices, privileged mode, host namespaces, Docker-socket access,
unconfined security or other separately classified host-equivalent features.

## Options and rationale

Keeping the fingerprint approval, deleting the entire host-access approval
system, and exempting only declared host mounts plus added capabilities were considered.
...
developer accepts that either declared bind representation exposes its selected
host path with the declared mount mode and that an added Linux capability expands the
...
Tests seal service-bind-only, volume-driver-bind-only, capability-only and
combined models without approval state, then prove enrollment, resealing,
first-use bootstrap and replay succeed. The effective evidence continues to
report all three risk categories. Negative tests
