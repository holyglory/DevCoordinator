...
The capability fixture now builds its relocation graph only through public
normalized observe, register, stop, and relocate actions. Pure inventory is
required to preserve the last proved lifecycle without host sampling; an
incapable status observation persists an explicit unknown identity without
releasing the lease, while a target-scoped capable inventory must recover exact
PID, address, and socket-inode proof. Before any guard operation is journaled,
the normalized action path imports pending authority, checks the local-host
installation fence and existing operation conflict, then samples relevant
listener identity. Exact assigned, singleton-range, and manual-lease ports are
covered even when no server row exists or a retained PID is dead. Independent
port-lease guards retain their bounded queue semantics. The Linux fixture also
proves the observer capability is present before exec, adds bounded subprocess
timeouts, and uses pidfd-bound cleanup plus a stable `/proc` no-match sweep so a
failed or disconnected start cannot leak a managed child or redirect a cleanup
signal after PID reuse.
