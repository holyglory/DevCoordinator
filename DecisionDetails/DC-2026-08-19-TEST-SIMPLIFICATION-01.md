...
- Keep testd, the root snapshot helper, transient non-root systemd attempts, exact repository/attempt identities, source containment, secret transport, bounded results, idempotent host operations, TTL containment, and explicit cancellation.
...
- Lifecycle tests prove testd determines terminal meaning, drains ordered result chunks, requests exact privileged collection, enforces semantic deadlines, and retains systemd TTL as independent containment.
