...
bind mount classified as `host_bind_mount`, an equivalent local volume-driver
bind classified as `volume_driver_bind`, or `cap_add`, classified as
`added_capabilities`. All three remain visible in the complete effective-model
risk evidence.
...
access; unconfined security; external containers, networks, or volumes; and any
other approval-required risk. Use the immutable
...
explicit approval operation; adding only `host_bind_mount`,
`volume_driver_bind`, or `added_capabilities` does not.
`devcoordinator-authority-repository-repair` is
