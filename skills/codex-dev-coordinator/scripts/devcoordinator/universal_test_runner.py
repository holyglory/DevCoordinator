...
from devcoordinator.universal_test_drivers import (  # type: ignore[import-not-found]
    adapt_driver_invocation,
    dotnet_locked_restore_required as _dotnet_locked_restore_required,
    is_dotnet_project_invocation as _is_dotnet_project_invocation,
)
...
def run(
    ...
    reporter_drivers = ReporterDrivers(
        {"jsonl": _jsonl_cases, "junit": _junit_cases, "trx": _trx_cases}
    )
    environment = {
        "PATH": "/usr/local/bin:/usr/bin:/bin",
    ...
    if generated_reporter_artifacts and generated_kind == "trx":
        ...
        for (
            ...
            try:
                ...
                reporter_cases, reporter_failures = reporter_drivers.parse(
                    "trx",
                    generated_path,
                    artifact_id=artifact_id,
                    case_namespace=namespace,
        ...
        try:
            reporter_kind = (
                "trx" if descriptor.reporter == "trx" else generated_kind
            )
            cases, reporter_failures = reporter_drivers.parse(
                reporter_kind, reporter_path
            )
            failures.extend(reporter_failures)
        except Exception as error:
            incomplete = True
