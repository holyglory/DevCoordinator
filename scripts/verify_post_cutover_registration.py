...
def verify_registration_graph(
    ...
    *,
    ...
    """Require one internally consistent assignment/server/lease graph."""
    ...
    # must not retain the checkout that was retired by this cutover.
    ...
    server_id_rows = [row for row in servers if row.get("id") == expected_server_id]
    if _one(server_id_rows, label="server row with the captured id") is not server:
        raise RegistrationGraphError("captured server id resolves to a different inventory row")
    ...
    target_active_leases = [
        ...
        if row.get("project") == project and row.get("server_id") == expected_server_id
    ]
    ...
    _require_equal(lease, "server_id", expected_server_id, label="active Console lease")
    ...
    return {
        ...
        "server_pid": main_pid,
        "lease_id": lease_id,
        "replacement_lease": lease_id != old_lease_id,
