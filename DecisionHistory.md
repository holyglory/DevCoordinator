...
Confirmed Console data direction: the server-side metrics loop is the attributed observer that advances current Docker presence and telemetry before a pure inventory read; live utilization is joined only by immutable resource identity inside the latest completed Docker-available snapshot; navigation collection badges count their named collection rather than a differently scoped attention queue. See [DC-2026-07-21-CONSOLE-DATA-01](DecisionDetails/DC-2026-07-21-CONSOLE-DATA-01.md).
