...
Confirmed operational direction: all enrolled users and agents on one host use one service-owned Coordinator authority through peer-authenticated authorization; clients do not open the authority database; test admission, repository attribution, run/case history, and statistics are universal Coordinator capabilities while test subprocesses remain under the enrolled user; new short-lived Docker workloads commit exact repository ownership, policy, and cleanup intent before create and start, while post-start attribution is recovery of that precommit rather than a name-derived claim; public and authenticated listeners remain independent availability boundaries during control-dependency maintenance, with supervised exits and durable journal evidence; authorization-schema upgrades migrate and reconcile protected profiles before a production restart, and long-lived loopback readers validate the protected profile before bind and supervised-restart after its stable identity changes; Docker routes survive container replacement without discarding retained history; a Google-protected route may translate the approved identity into either a private route credential or a short-lived signed route/method-bound identity assertion, but missing browser identity and rejected server-held credentials remain distinct failures, with exact return-to-page login for the former and branded HTML configuration failure for the latter; native work follows the Build macOS Apps workflow; and releases require remote-fresh, production-shaped evidence. See [DC-2026-07-26-AUTH-03](DecisionDetails/DC-2026-07-26-AUTH-03.md), [DC-2026-07-26-TEST-HARNESS-01](DecisionDetails/DC-2026-07-26-TEST-HARNESS-01.md), [DC-2026-07-25-AUTH-02](DecisionDetails/DC-2026-07-25-AUTH-02.md), [DC-2026-07-24-PROFILE-RELOAD-01](DecisionDetails/DC-2026-07-24-PROFILE-RELOAD-01.md), [DC-2026-07-23-EPHEMERAL-01](DecisionDetails/DC-2026-07-23-EPHEMERAL-01.md), [DC-2026-07-20-CONSOLE-RESILIENCE-01](DecisionDetails/DC-2026-07-20-CONSOLE-RESILIENCE-01.md), [DC-2026-07-19-ENROLLMENT-01](DecisionDetails/DC-2026-07-19-ENROLLMENT-01.md), [DC-2026-07-15-HOST-01](DecisionDetails/DC-2026-07-15-HOST-01.md), [DC-2026-07-16-ROUTES-01](DecisionDetails/DC-2026-07-16-ROUTES-01.md), [DC-2026-07-16-AUTH-01](DecisionDetails/DC-2026-07-16-AUTH-01.md), [DC-2026-07-10-07](DecisionDetails/DC-2026-07-10-07.md), and [DC-2026-07-11-19](DecisionDetails/DC-2026-07-11-19.md).

Confirmed route direction: the Console terminates public route TLS and forwards HTTP only to an explicitly selected application HTTP listener; it does not guess an upstream protocol from a port number. See [DC-2026-07-26-ROUTE-HTTP-01](DecisionDetails/DC-2026-07-26-ROUTE-HTTP-01.md).

Confirmed Console data and collection direction: the server-side metrics loop is the sole periodic observer; exact-project flights coalesce and back off without blocking host or pure committed-inventory sampling, and production background broker work begins only after independent registration readiness; live utilization is joined only by immutable resource identity inside the latest completed Docker-available snapshot; control-only server definitions remain available to the Ports workflow but never count as server instances without a concrete lifecycle observation; all host-scale resource-group headers remain discoverable while one bounded group opens at a time; cleanup controls remain fail-closed until their whole host capability is explicitly activated; and navigation collection badges count their named collection rather than a differently scoped attention queue. See [DC-2026-07-24-SERVER-PROJECTION-01](DecisionDetails/DC-2026-07-24-SERVER-PROJECTION-01.md), [DC-2026-07-22-CONSOLE-UI-01](DecisionDetails/DC-2026-07-22-CONSOLE-UI-01.md), [DC-2026-07-22-OBSERVATION-01](DecisionDetails/DC-2026-07-22-OBSERVATION-01.md), and [DC-2026-07-21-CONSOLE-DATA-01](DecisionDetails/DC-2026-07-21-CONSOLE-DATA-01.md).
...
ID: DC-2026-07-26-AUTH-03 · Details: [supporting record](DecisionDetails/DC-2026-07-26-AUTH-03.md)

Decision: For an explicitly configured protected development route whose application needs attributable controls, let the Console issue a 15-second Ed25519-signed assertion only after its existing Google identity and exact route grant pass. Bind the assertion to issuer, public host, route grant, HTTP method, normalized email, and one-time identifier; strip caller assertions on every route, publish only the public JWKS, and require the upstream to validate signature, claims, and replay before mapping permissions.

Why: A generic trusted identity header was previously rejected because a loopback caller could forge it, while a fixed private route credential proves only the Console service and cannot identify the human performing a mutation. Keeping a second application login defeats the requested single Console boundary, and giving every authorized user one shared audit actor makes full controls dishonest. A short-lived asymmetric, method-bound, replay-protected assertion preserves the Console's exact-resource authorization, gives the application a real actor, avoids sharing signing secrets, and remains reversible per route without weakening production authentication.

## DC-2026-07-26-TEST-HARNESS-01 — Testing is a repository-scoped Coordinator capability

ID: DC-2026-07-26-TEST-HARNESS-01 · Details: [supporting record](DecisionDetails/DC-2026-07-26-TEST-HARNESS-01.md)

Decision: Make the DevOps Coordinator the universal owner of test admission, repository attribution, durable session/group/individual-case results, and statistics. Let repository-controlled Python clients execute only structured manifest commands under the enrolled user identity, return result summaries to agents, and project the same repository-scoped records into DevOps Console and DevOps Board.

Why: Repository-local SQLite stores and dashboards duplicate lifecycle, authorization, retention, and UI logic and cannot provide one repo-by-repo operational view; running arbitrary repository commands inside the privileged broker would violate the broker's typed no-host-process boundary. A central broker journal plus unprivileged manifest runner preserves full/partial/individual selection, exact timings, and universal visibility without giving clients a database handle or giving the service arbitrary command execution.

## DC-2026-07-26-ROUTE-HTTP-01 — Public TLS terminates before route HTTP

ID: DC-2026-07-26-ROUTE-HTTP-01 · Details: [supporting record](DecisionDetails/DC-2026-07-26-ROUTE-HTTP-01.md)

Decision: Keep the Console as the public TLS terminator and forward routed HTTP and WebSocket traffic to the application's loopback HTTP listener. Make that upstream protocol explicit in route creation, Docker port selection, and live route details; do not point an HTTP route at a TLS-only listener.

Why: `gf2.vr.ae` was assigned to GlobalFinance Caddy's internal `https://localhost` port and received plaintext HTTP, while the same deployment deliberately publishes port 80 for an external TLS-terminating proxy. Adding a second TLS hop would require private-CA, SNI, and upstream-Host policy and failed against this application's hostname contract; guessing protocol from a port would be unreliable. Selecting the declared HTTP listener preserves end-to-end public HTTPS, the application's public Host header, WebSockets, and the existing auditable single-edge design.

## DC-2026-07-25-AUTH-02 — Rejected private credentials render an operational page

ID: DC-2026-07-25-AUTH-02 · Details: [supporting record](DecisionDetails/DC-2026-07-25-AUTH-02.md)

Decision: Keep missing browser identity and rejected private upstream credentials as separate boundaries. Redirect browser navigations without a valid Console identity to Google sign-in with the exact requested URL as the return target. After route authorization, inject only the saved server credential; if the upstream rejects it, discard that response body and render a branded HTML 502 configuration page without forwarding JSON or an authentication challenge.

Why: PRTZN's independently rotated token drifted from the Console copy, and the authenticated browser received its raw JSON 401. Re-running Google login or presenting Basic auth cannot repair a stale server secret; removing the upstream token weakens a multi-user loopback service; forwarding the raw response exposes an API failure as the product page. Distinct login and configuration failures preserve one browser journey and both security boundaries.

## DC-2026-07-24-PROFILE-RELOAD-01 — Protected profile publication reloads loopback readers

ID: DC-2026-07-24-PROFILE-RELOAD-01 · Details: [supporting record](DecisionDetails/DC-2026-07-24-PROFILE-RELOAD-01.md)

Decision: In server-wide mode, make the loopback Coordinator API parse the exact protected client-profile identity before binding, watch that identity without reading private content, and exit once after a stable replacement so its existing supervisor reloads the current reader. Keep the public Console and server-wide broker running across that independent API restart.

Why: The profile gained additive ephemeral-policy fields after the long-lived API had imported an older strict parser, so every authenticated request returned HTTP 500 while anonymous health remained green. Leaving restart manual or weakening strict field validation would preserve the stale-reader failure or accept unreviewed authority data; restarting the broker or public Console would broaden the blast radius. Exact startup validation plus a debounced identity-triggered API restart closes the rolling-publication gap while retaining strict parsing and independent availability boundaries.

## DC-2026-07-24-SERVER-PROJECTION-01 — Server collections require lifecycle evidence

ID: DC-2026-07-24-SERVER-PROJECTION-01 · Details: [supporting record](DecisionDetails/DC-2026-07-24-SERVER-PROJECTION-01.md)

Decision: Keep enrollment-only server definitions as normalized control identities for exact port lease and assignment authorization, but exclude them from operational Servers, project membership, running counts, Unassigned Resources, and server actions until a concrete running, starting, unhealthy, stopping, or stopped lifecycle observation exists. Make the Console independently fail closed on unknown or unobserved server lifecycle values during a rolling broker deployment.

Why: Treating an enabled ACL, port policy, assignment, or lease target as a current server was tried and failed: seven GlobalFinance validation-port identities appeared as `unobserved` servers despite a fresh host observation proving no PID, listener, port, URL, log, or operational status. Deleting the definitions would break intentional smoke-test port coordination, hiding them as user preferences would preserve false server semantics, and role-name filtering would encode a project convention. Separating control definitions from observed instances preserves exact authorization and history while making the named collections truthful.

## DC-2026-07-23-EPHEMERAL-01 — Ephemeral Docker ownership is committed before start

ID: DC-2026-07-23-EPHEMERAL-01 · Details: [supporting record](DecisionDetails/DC-2026-07-23-EPHEMERAL-01.md)

Decision: Admit new short-lived agent containers only through administrator-sealed repository templates and the peer-authenticated server-wide broker. Commit the exact UID/account, repository/template identities, unguessable creation nonce, immutable definition, TTL, quota, lease, and cleanup policy before Docker create; create stopped, persist the full immutable ID, then start. Permit post-start recording only as recovery when all precommitted labels match; require fresh immutable resource/controller/ownership fingerprints for explicit administrator attachment of an unrelated existing container. Make cleanup intent dominate every recovery path and exclude broker-owned ephemeral containers from generic Docker lifecycle, archive, and route controls.

Why: The observed start-before-durable-ownership path left `gnt-artifact-pg` running without durable membership, while name-derived repair could let one account claim another account's container. Name matching, post-hoc full-ID recording without prior authorization, and parallel raw Docker admission do not close the unattributed-running window; refusing all post-call recording would strand a valid create whose reply was lost. A durable nonce-and-label precommit plus stopped creation provides exact recovery without broadening ownership, and explicit immutable administrator attachment remains available for legacy unlabeled resources.

## DC-2026-07-22-CONSOLE-UI-01 — Resource groups disclose locally and responsive rows protect identity

ID: DC-2026-07-22-CONSOLE-UI-01 · Details: [supporting record](DecisionDetails/DC-2026-07-22-CONSOLE-UI-01.md)

Decision: Present every nonempty Servers and Docker project as a collapsed-by-default accessible disclosure, keep only one project open per collection, and page only that project's visible resources at 75 rows. Protect the server identity with a two-row layout from 720–1023px and the container name from 720–1279px. At 480–719px, group each server's identity, secondary facts, and actions into three compact bands without redundant field-label lines; below 480px use four bounded bands and let the sparkline yield to its live values. Treat cleanup as a separately activated Console capability, default it off, expose its readiness through the session, and reject every lifecycle API call server-side until explicitly enabled after the matching broker migration and authorization are ready.

Why: Flat host-wide lists and one-row fixed-column layouts were tried and failed: an 85-container GlobalFinance group hid later project headers, six Docker side columns reduced a long name to one character per line at 1135px, and the analogous Servers row collapsed `smoke-caddy-http` vertically at 787px because port, telemetry, status, warning, and actions exhausted the row before its mobile breakpoint. The first mobile Servers card also failed by placing every secondary field and generated label on its own line, making a 620px row several hundred pixels tall despite sparse content. The first responsive Docker guard used empty metrics at 390px, so a fixed 92px SVG plus nowrap values and a horizontal generated label escaped the card unnoticed. Cross-project paging, project-only filtering, clipping charts, removing technically available columns, or retaining one-field-per-line mobile cards would hide real resources/data or waste the primary viewport; persistent expansion would conceal changing operational state. Separately, granting only `archives.read` or treating Google ownership as cleanup authority would partially bypass a production migration still blocked on terminal database timeouts and restore replay. Transient group-local disclosure, width-specific compact geometry with real rendered-data guards, and one fail-closed whole-capability gate keep every project discoverable without claiming unsafe cleanup readiness.

## DC-2026-07-22-OBSERVATION-01 — Metrics is the sole periodic host observer

ID: DC-2026-07-22-OBSERVATION-01 · Details: [supporting record](DecisionDetails/DC-2026-07-22-OBSERVATION-01.md)

Decision: Keep explicit host observation at the Telegram authorization boundary, but remove Telegram's periodic observer and let it consume durable events thereafter. Coalesce concurrent Console observations by exact project, give the serialized host-wide operation a 12-minute client deadline, and exponentially back off metrics observation failures from one sampling interval to a five-minute cap while machine and committed-inventory sampling continue every tick. In required production startup, defer all background broker loops beyond independent registration readiness and give its single inventory request the remaining global deadline.

Why: The earlier 60-second client deadline and independent Telegram five-second observer were tried and failed under production broker contention: timed-out Node requests left broker work running, Telegram immediately launched more, and the four-caller ceiling then rejected Console work including archives. A separate three-second readiness cap also failed: it abandoned valid full-inventory reads, accumulated server work, and caused 84 supervised restart attempts after the Console had already registered. Restarting the broker would cross unresolved quiescence and database-replay blockers; disabling observation would make Docker presence stale; eager or short-capped retries would recreate the storm. One coalesced observer, completion-anchored backoff, readiness-first background startup, and one globally bounded readiness read preserve truthful inventory without multiplying work.
...
Decision: Make the Console metrics loop explicitly request an attributed host observation before each pure inventory read, retain the last committed inventory while reporting observation failure, and give host observation a Docker-sized client deadline. Project live container telemetry into the Console compatibility read model only for an exact running immutable resource sampled within its host's latest completed Docker-available snapshot; during independent broker/API rollout, fill a missing compatibility value only when the authenticated observation result proves that exact normalized window and never override a canonical value. Count registered bots in the Telegram navigation badge; keep pending Telegram users only in explicitly labeled authorization queues.

Why: The prior pure-read sampler was tried and failed: removed containers could remain visible until an unrelated agent or eligible Telegram recipient observed the host, while normalized telemetry existed but never reached Docker rows, and the generic 15-second observation deadline repeatedly timed out in production. Removing the CPU/memory column would discard technically available exact telemetry; UI-side status/name filtering or joining old samples would invent current state; making ordinary inventory mutate would violate its query-only contract; and showing pending-user count in a badge named Telegram misrepresented the bot collection. One attributed observer plus snapshot-bound producer projection and collection-aligned counts preserves truth, retained history, and clear semantics.
