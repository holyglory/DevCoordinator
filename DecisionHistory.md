...
Decision: Before an offline server-wide authority migration, publish one root-owned, deployment-ID-bound maintenance marker outside the broker systemd runtime directory. Every broker transport checks it before socket access and returns a typed bounded wait response; malformed state fails closed. Keep the API and public Console independently supervised, run broker stop, migration, restart, and verification inside one foreground rollback transaction, and clear only after exact health, authentication, inventory, Console registration, assignment, lease, and public-route proof—or after an equally verified rollback.

Why: A generic connection failure leaves agents retrying blindly, while a marker beside the broker socket disappears on intentional systemd stop and a database-backed flag becomes unreadable during the migration it must describe. Stopping the full public stack creates unnecessary outage, and clearing on process start hides partial recovery. A separate protected runtime fence costs one tmpfiles-managed directory and client check but survives broker removal, cannot be cleared by another deployment, communicates an actionable wait to all cooperative clients, and preserves the independently available Console/API boundaries without weakening rollback gates.
